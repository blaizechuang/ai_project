...
if __name__ == '__main__':
    ...
    train = pd.read_csv('input/digit_recognizer/train.csv')
    labels = train.ix[:, 0].values.astype('int32')
    X_train = (train.ix[:, 1:].values).astype('float32')
    X_test = (pd.read_csv('input/digit_recognizer/test.csv').values).astype('float32')
    ...
    result.to_csv("input/digit_recognizer/pred_mnist.csv",
